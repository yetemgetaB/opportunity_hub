...
describe('User + Role Database Integration (Backend 1 <-> Backend 2 Contract)', () => {
    ...
    mockPrisma = {
      user: {
        ...
        findFirst: jest.fn().mockImplementation(async ({ where }) => {
          ...
          if (!user) return null;
          if (where.isActive !== undefined && user.isActive !== where.isActive) return null;
          if (where.deletedAt === null && user.deletedAt !== null) return null;
          return user;
        }),
      },
    };
  ...
  it('Flow: User registers via Supabase Auth -> App user record created -> Role retrieved by Auth ID', async () => {
    // 1. Supabase Auth generates a UUID for the newly registered student
    const supabaseAuthUserId = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11';
    ...
    // 3. Subsequent request: Backend 1 inspects JWT sub (auth UID) and retrieves role
    const roleInfo = await usersService.getRoleByAuthId(supabaseAuthUserId);
  ...
  it('Security: Duplicate user record creation with same Auth ID must be rejected', async () => {
    const supabaseAuthUserId = 'b1ffcd88-8b1a-3ef7-aa5c-5aa8ac270b22';
  ...
  it('Security: Only valid platform roles (STUDENT, ORGANIZATION, ADMIN) are accepted', async () => {
    const invalidId = 'c2eedc77-7a2b-2ef6-994b-4997ab160c33';
